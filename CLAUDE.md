...
### 移動中に脱出アニメーションに遷移できないバグ（2026-01-18）

**症状**: 
- 最終クイズ正解後、プレイヤーが移動中だとアニメーション画面に遷移できない
- 正解してからアニメーションまでの待ち時間が長い

**原因**:
- Supabase保存とランキング取得のawaitが画面遷移をブロック
- Three.jsのアニメーションループが移動処理を続行
- isEscapingフラグがなく、正解後も移動が可能

**修正内容**:
1. `isEscaping`ステートと`isEscapingRef`を追加（82-83行目）
2. 正解時に即座にローディングオーバーレイを表示（3366-3375行目）
3. アニメーションループでisEscapingRefをチェックし、移動を停止（2615-2620行目）
4. fetchOtherPlayersでも同様にisEscapingを設定（1327-1328行目）
...
### 管理画面ランキング編集機能（2026-01-18）

**機能追加**: 管理者がランキングを手動で編集・修正できる機能

**背景**: DBの反映遅延により、チームの完了時間がランキングに正しく表示されない場合がある

**追加機能**:
1. **クリア済みチームのタイム編集**: ランキングテーブルの「操作」列から完了時間を変更可能
2. **完了状態リセット**: クリア済みチームを未クリア状態に戻す
3. **未クリアチームへの手動クリア設定**: プレイ中チームに手動でクリアタイムを設定

**追加された関数**:
- `fixTeamCompletion(teamCode, timeString)`: チームの完了時間を手動設定（MM:SS形式）
- `resetTeamCompletion(teamCode)`: チームの完了状態をリセット

**追加されたステート**:
- `editingTeamCode`: 編集中のチームコード
- `editingTime`: 編集中の時間入力値

**使用方法**:
1. 管理画面（パスワード: admin123）に入る
2. ランキングセクションで「編集」ボタンをクリック
3. MM:SS形式で時間を入力して「保存」
4. 未クリアチームは「クリアタイムを設定」ボタンから設定可能

**関連コード**: 
- 473-529行目付近: fixTeamCompletion, resetTeamCompletion関数
- 3085-3150行目付近: ランキングテーブルUI
- 3277-3310行目付近: チームカード内の手動クリア設定UI
